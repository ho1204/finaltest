...
export const WEAPONS = {
    default: {
        ...
        fire({ player, Bullet, groupId }) {
            ...
            // 기본 패턴: 3발 이상이면 원형 클러스터, 1~2발은 가로 퍼짐
            if (count >= 3) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    bullets.push(
                        new Bullet(bx, by, dmg, groupId, color, "default", 0, -15)
                    );
                }
            } else {
                for (let i = 0; i < count; i++) {
                    ...
                    bullets.push(
                        new Bullet(player.x + spread, player.y - 20, dmg, groupId, color, "default", 0, -15)
                    );
                }
            }
    ...
    shotgun: {
        ...
        fire({ player, Bullet, groupId }) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                bullets.push(
                    new Bullet(player.x, player.y - 20, dmg, groupId, color, "shotgun", vx, vy)
                );
            }
        ...
        fire({ player, Bullet, groupId }) {
            ...
            if (count >= 3) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    const by = player.y - 20 + Math.sin(angle) * clusterRadius;
                    bullets.push(
                        new Bullet(bx, by, dmg, groupId, color, "shuriken", 0, vy)
                    );
                }
            } else {
                for (let i = 0; i < count; i++) {
                    const spread = (i - (count - 1) / 2) * 8;
                    bullets.push(
                        new Bullet(player.x + spread, player.y - 20, dmg, groupId, color, "shuriken", 0, vy)
                    );
                }
            }
